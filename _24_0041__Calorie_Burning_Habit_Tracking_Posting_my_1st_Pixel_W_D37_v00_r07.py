...
posting_a_pixel_endpoint = f"{pixela_endpoint}/{USERNAME}/graphs/{GRAPH_NAME}"   # update our endpoint URL, by adding what we had previously, but add "/graphs/graph1"
...
# custom_date = datetime(year=2024, month=7, day=23)
...
posting_a_pixel_config = {
    "date": todays_formatted_date,  #format: YYYYMMDD
    "quantity": "323",   # in calories
    "optionalData": json.dumps("Walk/hike with Bets at a nearby park that Bets likes")     # json.dumps() with an s. The s indicates it should be a String JSON format.
}
